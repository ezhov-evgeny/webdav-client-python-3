...
class ClientTestCase(BaseClientTestCase):
    pulled_file = (
        BaseClientTestCase.local_path_dir + os.sep + BaseClientTestCase.local_file
    )
    ...
    def test_list(self):
        ...
        self.assertGreater(
            file_list.__len__(), 0, "Expected that amount of files more then 0"
        )
    ...
    def test_list_no_parent(self):
        ...
        for file_name in file_list:
            self.assertNotEqual(
                self.remote_path_dir,
                file_name,
                "Result should not contain parent directory",
            )
    ...
    def test_list_info(self):
        ...
        self.assertTrue(
            "modified" in list_info[0].keys(), "info should contain modified"
        )
        ...
        self.assertTrue(
            "content_type" in list_info[0].keys(), "info should contain content_type"
        )
    ...
    def test_check(self):
        ...
        self.assertTrue(
            self.client.check(self.remote_path_dir),
            "Expected that the directory is exist",
        )
        self.assertTrue(
            self.client.check(self.remote_inner_path_dir),
            "Expected that the inner directory is exist",
        )
        self.assertTrue(
            self.client.check(self.remote_path_file), "Expected that the file is exist"
        )
        self.assertTrue(
            self.client.check(self.remote_inner_path_file),
            "Expected that the inner file is exist",
        )

    def test_check_does_not_exist(self):
        self._prepare_for_downloading(inner_dir=True)
        self.assertFalse(
            self.client.check("wrong"), "Expected that the directory is not exist"
        )
        self.assertFalse(
            self.client.check(self.remote_path_dir + "/wrong"),
            "Expected that the inner directory is not exist",
        )
        self.assertFalse(
            self.client.check(self.remote_path_dir + "/wrong.txt"),
            "Expected that the file is not exist",
        )
        self.assertFalse(
            self.client.check(self.remote_inner_path_dir + "/wrong.txt"),
    ...
    def test_check_another_client(self):
        ...
        self.client.upload_sync(
            remote_path=self.remote_path_file, local_path=self.local_path_dir
        )
    ...
    def test_download_from_compressed(self):
        self._prepare_dir_for_downloading(
            self.remote_path_dir,
            self.remote_compressed_path_file,
            local_file_path=self.local_compressed_file_path,
        )
        buff = BytesIO()
        self.client.download_from(
            buff=buff, remote_path=self.remote_compressed_path_file
        )
    ...
    def test_download_directory_wrong(self):
        ...
        with self.assertRaises(RemoteResourceNotFound):
            self.client.download_directory(
                remote_path="wrong", local_path=self.local_path_dir
            )

    def test_download(self):
        self._prepare_for_downloading()
        self.client.download(
            local_path=self.local_path_dir, remote_path=self.remote_path_dir
        )
        self.assertTrue(
            path.exists(self.local_path_dir), "Expected the directory is downloaded."
        )
        self.assertTrue(
            path.isdir(self.local_path_dir), "Expected this is a directory."
        )
        self.assertTrue(
        ...
        self.assertTrue(
            path.isfile(self.local_path_dir + os.path.sep + self.local_file),
    ...
    def test_download_sync(self):
        ...
        self.client.download_sync(
            local_path=self.local_path_dir + os.path.sep + self.local_file,
    ...
    def test_upload_from(self):
        self._prepare_for_uploading()
        buff = StringIO()
        buff.write("test content for testing of webdav client")
        self.client.upload_to(buff=buff, remote_path=self.remote_path_file)
        self.assertTrue(
            self.client.check(self.remote_path_file), "Expected the file is uploaded."
        )
    ...
    def test_upload_file(self):
        ...
        self.client.upload_file(
            remote_path=self.remote_path_file, local_path=self.local_file_path
        )
        self.assertTrue(
            self.client.check(remote_path=self.remote_path_file),
    ...
    def test_upload_sync(self):
        ...
        def callback():
            ...
            self.assertTrue(
                self.client.check(self.remote_path_file),
                "Expected the file is uploaded.",
            )
    ...
    def test_copy(self):
        ...
        self.client.copy(
            remote_path_from=self.remote_path_file,
    ...
    def test_directory_is_dir(self):
        self._prepare_for_downloading()
        self.assertTrue(
            self.client.is_dir(self.remote_path_dir), "Should return True for directory"
        )

    def test_file_is_not_dir(self):
        self._prepare_for_downloading()
        self.assertFalse(
            self.client.is_dir(self.remote_path_file), "Should return False for file"
        )

    def test_get_property_of_non_exist(self):
        self._prepare_for_downloading()
        result = self.client.get_property(
            remote_path=self.remote_path_file, option={"name": "aProperty"}
        )
        self.assertEqual(
    ...
    def test_set_property_batch(self):
        self._prepare_for_downloading()
        self.client.set_property_batch(
            remote_path=self.remote_path_file,
            option=[
                {
                    "namespace": "http://test.com/ns",
    ...
    def test_pull(self):
        ...
        self.assertTrue(
            path.exists(self.local_path_dir + os.path.sep + self.inner_dir_name),
        ...
        self.assertTrue(
            path.isdir(self.local_path_dir + os.path.sep + self.inner_dir_name),
    ...
    def test_pull_newer(self):
        init_modification_time = int(
            self._prepare_local_test_file_and_get_modification_time()
        )
        sleep(1)
    ...
    def test_push_newer(self):
        ...
        sleep(1)
        self._prepare_for_uploading()
        init_modification_time = self.client.info("time/" + self.remote_path_file)[
            "modified"
        ]
        result = self.client.push("time/" + self.remote_path_dir, self.local_path_dir)
        update_modification_time = self.client.info("time/" + self.remote_path_file)[
            "modified"
        ]
        self.assertTrue(result)
