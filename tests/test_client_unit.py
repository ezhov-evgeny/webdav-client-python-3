...
class ClientTestCase(TestCase):
    def test_parse_get_list_response(self):
        ...
        self.assertTrue(
            result[0].is_dir(),
            "{} should be marked as directory".format(result[0].path()),
        )
        self.assertFalse(
            result[1].is_dir(), "{} should be marked as file".format(result[1].path())
        )
    ...
    def test_parse_get_list_response_dirs(self):
        ...
        for d in result:
            self.assertTrue(
                d.is_dir(), "{} should be marked as directory".format(d.path())
            )
    ...
    def test_parse_free_space_response_not_supported(self):
        ...
        self.assertRaises(
            MethodNotSupported, Utils.parse_free_space_response, content, "localhost"
        )
    ...
    def test_get_info_from_response(self):
        content = read_file_content("./tests/responses/get_info.xml")
        response = Utils.extract_response_for_path(
            content, "/test_dir/test.txt", "localhost"
        )
    ...
    def test_create_get_property_request_content(self):
        ...
        result = Utils.create_get_property_request_content(
            option=option,
        )
        self.assertEqual(
            result,
    ...
    def test_create_set_one_property_request_content(self):
        option = {"namespace": "test", "name": "aProperty", "value": "aValue"}
    ...
    def test_create_set_property_batch_request_content(self):
        options = [
            ...
            {"namespace": "test2", "name": "aProperty2", "value": "aValue2"},
        ]
    ...
    def test_create_set_property_batch_request_content_name_only(self):
        options = [{"name": "aProperty"}, {"name": "aProperty2"}]
        result = Utils.create_set_property_batch_request_content(options=options)
        self.assertEqual(
            result,
            b"<?xml version='1.0' encoding='UTF-8'?>\n<propertyupdate xmlns=\"DAV:\"><set><prop>"
    ...
    def test_parse_is_dir_response_not_supported(self):
        content = read_file_content(
            "./tests/responses/is_dir_directory_not_supported.xml"
        )
        path = "/test_dir"
        hostname = "https://webdav.yandex.ru"
        self.assertRaises(
            MethodNotSupported, Utils.parse_is_dir_response, content, path, hostname
        )
    ...
    def test_extract_response_for_path_not_supported(self):
        self.assertRaises(
            MethodNotSupported,
            Utils.extract_response_for_path,
            "WrongXML",
            "test",
            "https://webdav.ru",
        )
    ...
    def test_response_error_code(self, mock_session):
        ...
        self.assertRaises(
            ResponseErrorCode, client.execute_request, action="list", path=""
        )
    ...
    def test_method_not_supported(self, mock_session):
        ...
        self.assertRaises(
            MethodNotSupported, client.execute_request, action="list", path=""
        )
    ...
    def test_not_found(self, mock_session):
        ...
        self.assertRaises(
            RemoteResourceNotFound, client.execute_request, action="list", path=""
        )
    ...
    def test_not_enough_space(self, mock_session):
        ...
        self.assertRaises(
            NotEnoughSpace, client.execute_request, action="list", path=""
        )
